...
module RecordingStudioCategorisable
  class ApplicationController < ::ApplicationController
    ...
    helper_method :current_root_recording
    ...
    def recording_label(recording)
      ...
      return "#{recording.recordable_type.to_s.demodulize} (missing)" if recordable.blank?
    ...
    def supplemental_items_for_group(group_key:, current_root:, existing_item_keys:)
      ...
      source_groups.each do |group_recording|
        ...
        item_recordings.each do |item_recording|
          ...
          next unless existing.blank? || item_recording.created_at > existing[:created_at]
      ...
      candidates_by_key.values
                       .sort_by { |entry| [entry[:item][:name].to_s.downcase, entry[:item][:key].to_s] }
