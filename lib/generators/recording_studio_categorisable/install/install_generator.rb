...
module RecordingStudioCategorisable
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      desc "Installs RecordingStudioCategorisable engine into your application"

      class_option(
        :mount_path,
        type: :string,
        default: "/recording_studio_categorisable",
        desc: "Route prefix used when mounting the engine"
      )

      def mount_engine
        route %(mount RecordingStudioCategorisable::Engine, at: "#{options[:mount_path]}")
      end

      def copy_initializer
        template "recording_studio_categorisable_initializer.rb",
      ...
      def add_tailwind_source
        ...
        if missing_lines.empty?
          say "Tailwind already configured to include RecordingStudioCategorisable and FlatPack sources.", :green
          return
        end
      ...
      def inject_tailwind_sources(tailwind_css_path, missing_lines)
        ...
        say "Added RecordingStudioCategorisable and FlatPack sources to Tailwind CSS configuration.", :green
        say "Run 'bin/rails tailwindcss:build' to rebuild your CSS.", :green
      end

      def formatted_tailwind_source_block(missing_lines)
        [
          "\n/* Include RecordingStudioCategorisable engine views for Tailwind CSS */",
      ...
      def tailwind_source_lines
        [
          ...
          bundler_gem_views_source,
          '@source "../../vendor/bundle/**/flatpack/app/components/**/*.{rb,erb}";',
          bundler_gem_components_source
        ]
      end

      def bundler_gem_views_source
        '@source "../../../../../../usr/local/bundle/ruby/**/bundler/gems/' \
          'recording_studio_categorisable-*/app/views/**/*.erb";'
      end

      def bundler_gem_components_source
        '@source "../../../../../../usr/local/bundle/ruby/**/bundler/gems/flatpack-*/' \
